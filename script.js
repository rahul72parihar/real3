...
// Preloader + orchestrated hero reveal on site load
...
if (preloader) {
  ...
  const revealHero = () => {
    ...
    if (heroMedia) heroMedia.classList.add('revealed');

    const items = document.querySelectorAll('.hero-reveal-item');
    items.forEach((el, i) => {
      setTimeout(() => el.classList.add('visible'), reducedMotion ? 0 : 160 + i * 110);
    });

    setTimeout(() => preloader.remove(), reducedMotion ? 0 : 700);
  };
  ...
  if (reducedMotion) {
    ...
    setTimeout(revealHero, 2800); // fallback in case load never fires promptly
  }
}
